...
#include "filter.h" // aviutlのoriginalのfilter
...
/*
inline FILTER_DLL *Exedit_GetFilter(FILTER *fp)
{
	Filter *f = new Filter(fp);
	for(int i = f->getFilterCount(); i;) {
...
struct EXEDIT_OBJECT {
	...
	int  frame_begin; // 動画を通しての開始位置
	int  frame_end;  // 動画を通しての終了位置
	...
	FILTER_PARAM filter_param[MAX_FILTER]; // memo trackbar?
	// filterのidを含む
...
struct EXEDIT_FILTER {
  ...
  // この後ろが微妙に違うな hwndの後、dll_instance HINSTANCE -> win instance handle
	void   *exedit_exfunc; // hwndのはず unsigned integer4バイト pointerも４バイト
	void   *dll_inst; // 常にNULL
	void   *exdata_def; // 0x6C : 拡張データの初期値
  // sdkだとこの後ろはfunc is save frameなどのcallback funcへのポインタが定義されてる
	...
	EXEDIT_OBJECT *object_data; // 0xE0 memo おそらくexedit winに表示するobjの情報
  // memo しかしここにはtextは保存されていないっぽい
