    ...
    <script language="JScript">
       ...
       if( ret === true ) {
         ...
         if( ret === true) {
           ...
           if(i === 0) {
             // var ret = shell.Popup(".NET4.8はインストール済みです",5,"Info",0);
...
// installed = false for debug
       if( installed === false)
       {
         ...
         if ( ret !== 2) {
           ...
           var ret = shell.Popup("exe:" + file + "を実行しますか？",10,"処理確認",33);
           if ( ret !== 2) {
       ...
       // 同じフォルダのzipをprogram配下に展開？
       // カレントに展開 -> exe実行 yes or no
       var ret = shell.Popup("Saltstoneインストールを実行しますか？",10,"処理確認",33);
       if ( ret == 2) {
         WScript.Quit(0);
       }
